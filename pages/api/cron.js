...
Identifiziere die 2 wichtigsten und meistberichteten Themen für: "${category.label} – ${category.sub}".

MINDEST-QUELLEN-REGEL — KRITISCH:
Wähle NUR Themen über die MINDESTENS 3 verschiedene Quellen berichtet haben.
Themen mit nur 1-2 Quellen sind zu nischig — ignoriere sie.
Wähle immer die Themen mit der höchsten Quellenanzahl.

DIVERSITÄTS-REGEL:
Die 2 Themen müssen UNTERSCHIEDLICHE Ereignisse sein — verschiedene Länder, Schauplätze, Akteure.
...
async function agent2_extractFacts(topic, rssArticles, webArticles = []) {
  const today = new Date().toLocaleDateString("de-DE", { day:"2-digit", month:"2-digit", year:"numeric" });
  const sorted = [...rssArticles].sort((a, b) => (b.hasFullText ? 1 : 0) - (a.hasFullText ? 1 : 0));
  const fulltextCount = rssArticles.filter(a => a.hasFullText).length;

  // RSS-Artikel als Basis — kompakt
  const rssTexts = sorted.slice(0, 4).map(a =>
    `[${a.source}] ${a.title}: ${a.summary.slice(0, 200)}`
  ).join("\n---\n");

  // Web Search Artikel als Ergänzung — nur Kernaussagen
  const webTexts = webArticles.slice(0, 3).map(a =>
    `[${a.source}] ${a.title}: ${a.key_points?.slice(0,2).join(". ")}`
  ).filter(Boolean).join("\n---\n");
  ...
  try {
    const res = await withRetry(() => client.messages.create({
      model: "claude-haiku-4-5-20251001",
      max_tokens: 600,
      system: `Fakten-Extraktor. Datum: ${today}
Schreibe eine sachliche deutsche Zusammenfassung.
Headline: max 10 Wörter, deutsch, nie "Keine Artikel verfügbar".
Summary: 2-3 Sätze mit wichtigsten Fakten.
Antworte NUR mit JSON: {"headline":"...","summary":"...","confidence":"high"}`,
      messages: [{ role: "user", content: `Thema: ${topic.topic}\n\nRSS:\n${rssTexts || "Keine"}\n\nWeb:\n${webTexts || "Keine"}` }]
    }));
    const result = extractJSON(res.content[0].text);
    // Validierung
    if (!result.headline || result.headline.toLowerCase().includes("keine")) {
      result.headline = topic.topic;
    }
    if (!result.summary || result.summary.length < 20) {
      result.summary = webTexts ? webTexts.slice(0, 200) : `Aktuelle Entwicklungen: ${topic.topic}`;
    }
    return result;
  } catch {
    // Robuster Fallback — niemals Fehler werfen
    return {
      headline: topic.topic,
      summary: webArticles.length > 0
        ? webArticles.slice(0,2).map(a => a.key_points?.[0]).filter(Boolean).join(" ")
        : `Aktuelle Berichterstattung zu: ${topic.topic}`,
      confidence: "low"
    };
  }
}
...
async function agent3a_searchLeft(topic, category) {
  ...
  const isBoerse = category.id.includes("boerse");

  // Breite Suchanfragen ohne Kategorie-Einschränkung
  // Gleiche Suchanfragen für alle Kategorien damit dasselbe Thema
  // in verschiedenen Kategorien dieselben Quellen findet
  const searches = isGerman ? [
    `${topic.topic} 2026`,
    `${topic.topic} Kommentar Analyse`,
  ] : [
    `${topic.topic} 2026`,
    `${topic.topic} analysis comment`,
  ];
  ];
  ...
  const res = await withRetry(() => client.messages.create({
    ...
    system: `Du suchst nach Artikeln von links-liberalen Medien zu einem Nachrichtenthema. (Datum: ${today})

Führe BEIDE Suchen durch und analysiere alle Treffer:
1. ${searches[0]}
2. ${searches[1]}

PRIORISIERUNG der Quellen:
1. Bevorzuge: Spiegel, taz, Süddeutsche, Zeit, Tagesspiegel, Stern, Frankfurter Rundschau, Guardian, NYT, NPR, Washington Post, Le Monde, BBC
2. Falls diese nicht gefunden: Nimm andere seriöse Nachrichtenmedien aus den Ergebnissen
3. IMMER mindestens 1-2 Artikel zurückgeben — auch wenn nicht von der Wunschliste

IGNORIERE NUR: RT, Breitbart, Parteiseiten, reine Blogs.

Gib NUR valides JSON zurück:
{"articles":[{"source":"Spiegel","title":"Artikeltitel","key_points":["Kernaussage 1","Kernaussage 2"]}],"search_successful":true}`,
    messages: [{
      role: "user",
      content: `Thema: ${topic.topic}\nDatum: ${today}\n\nFühre die Suchen durch. Gib IMMER Artikel zurück — auch wenn sie nicht von deinen bevorzugten Quellen stammen.`
    }]
  }));
  ...
  try {
    ...
    // Nur deduplizieren nach Source — kein Whitelist-Filter mehr
    const seen = new Set();
...
async function agent3b_searchRight(topic, category) {
  ...
  const res = await withRetry(() => client.messages.create({
    ...
    max_tokens: 1500,
    tools: [{ type: "web_search_20250305", name: "web_search", max_uses: 4 }],
    system: `Du suchst nach Artikeln von konservativen Medien zu einem Nachrichtenthema. (Datum: ${today})
...
PRIORISIERUNG der Quellen:
1. Bevorzuge: FAZ, Welt, NZZ, Focus, Cicero, Handelsblatt, Wirtschaftswoche, WSJ, Telegraph, Washington Examiner, The Times
2. Falls diese nicht gefunden: Nimm andere wirtschaftsliberale oder konservative Nachrichtenmedien
3. IMMER mindestens 1-2 Artikel zurückgeben — auch wenn nicht von der Wunschliste

IGNORIERE NUR: RT, Breitbart, reine Blogs, Parteiseiten.
Guardian, NYT, taz, Spiegel, BBC sind NIEMALS konservative Quellen.
...
async function generateCategoryBriefing(categoryId) {
  ...
  // Pro Thema: Neue Reihenfolge — Agent 3 zuerst, dann Agent 2 mit Web Search Ergebnissen
  const newsItems = [];
  for (const topic of topics) {
    console.log(`\nVerarbeite: "${topic.topic}"`);

    // Agents 3a + 3b ZUERST — Web Search parallel
    await pause(3000);
    const [leftSearch, rightSearch] = await Promise.all([
      agent3a_searchLeft(topic, category),
      agent3b_searchRight(topic, category),
    ]);
    console.log(`  Agent 3a: ${leftSearch.articles?.length || 0} linke Artikel (${leftSearch.articles?.map(a=>a.source).join(", ")})`);
    console.log(`  Agent 3b: ${rightSearch.articles?.length || 0} konservative Artikel (${rightSearch.articles?.map(a=>a.source).join(", ")})`);

    // Agent 2: Fakten — nutzt SOWOHL RSS als auch Web Search Ergebnisse
    // → Summary und Analyse kommen jetzt aus denselben Quellen
    await pause(5000);
    const topicArticles = allArticles.filter(a => topic.sources?.includes(a.source));
    const usedArticles = topicArticles.length >= 3 ? topicArticles : allArticles.slice(0, 8);
    const allSearchArticles = [
      ...leftSearch.articles || [],
      ...rightSearch.articles || [],
    ];
    const facts = await agent2_extractFacts(topic, usedArticles, allSearchArticles);

    // Verhindert "Keine Artikel verfügbar" als Headline
    if (!facts.headline || facts.headline.toLowerCase().includes("keine") || facts.headline.toLowerCase().includes("verfügbar")) {
      facts.headline = topic.topic;
    }
    // Summary aus Web Search wenn RSS kein gutes Material hat
    const badSummary = !facts.summary ||
      facts.summary.toLowerCase().includes("keine") ||
      facts.summary.toLowerCase().includes("verfügbar") ||
      facts.summary.toLowerCase().includes("behandeln folgende") ||
      facts.summary.toLowerCase().includes("volltexte behandeln") ||
      facts.summary.toLowerCase().includes("enthalten keine") ||
      facts.summary.length < 30;

    if (badSummary) {
      const webSummary = allSearchArticles
        .slice(0, 3)
        .map(a => a.key_points?.[0])
        .filter(Boolean)
        .join(" ");
      facts.summary = webSummary.length > 30
        ? webSummary
        : `Aktuelle Berichterstattung zu: ${topic.topic}`;
    }
    console.log(`  Agent 2: "${facts.headline}" (${facts.confidence})`);

    // Agents 4a + 4b SEQUENTIELL
    await pause(5000);
    const leftPerspective = await agent4a_writeLeftPerspective(topic, facts, leftSearch);
    console.log(`  Agent 4a: ${leftPerspective.sources_used?.join(", ")}`);

    await pause(5000);
    const rightPerspective = await agent4b_writeRightPerspective(topic, facts, rightSearch);
