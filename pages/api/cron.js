...
async function agent3a_searchLeft(topic, category) {
  ...
  const searches = isGerman ? [
    `${topic.topic} site:spiegel.de OR site:taz.de OR site:sueddeutsche.de`,
    `${topic.topic} site:zeit.de OR site:tagesspiegel.de OR site:fr.de`,
  ] : [
    `${topic.topic} site:theguardian.com OR site:nytimes.com OR site:npr.org`,
    `${topic.topic} site:washingtonpost.com OR site:lemonde.fr`,
  ];
...
{"articles":[{"source":"Spiegel","title":"...","key_points":["Punkt 1","Punkt 2"]}],"search_successful":true}`,
  ...
  try {
    ...
    const allowedLeft = ["Spiegel", "taz", "Süddeutsche", "Süddeutsche Zeitung", "Zeit", "Zeit Online",
      "Tagesspiegel", "Frankfurter Rundschau", "Stern", "Guardian", "The Guardian",
      "New York Times", "NPR", "Washington Post", "Le Monde", "Der Standard"];
...
async function agent3b_searchRight(topic, category) {
  ...
  // Präzise Suchanfragen mit bekannten Domains
  const searches = isGerman ? [
    `${topic.topic} site:faz.net OR site:welt.de OR site:nzz.ch`,
    `${topic.topic} site:focus.de OR site:cicero.de OR site:handelsblatt.com`,
  ] : [
...
{"articles":[{"source":"FAZ","title":"...","key_points":["Punkt 1","Punkt 2"]}],"search_successful":true}`,
  ...
  try {
    const result = extractJSON(lastText);
    // Filtere unbekannte Quellen
    const allowedRight = ["FAZ", "Frankfurter Allgemeine", "Welt", "Die Welt", "NZZ",
