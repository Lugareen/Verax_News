...
async function agent3a_searchLeft(topic, category) {
  ...
  // Präzise Suchanfragen mit bekannten Domains
  const searches = isGerman ? [
    `${topic.topic} site:spiegel.de OR site:taz.de OR site:sueddeutsche.de OR site:zeit.de`,
    `${topic.topic} site:tagesspiegel.de OR site:fr.de OR site:stern.de OR site:sz.de OR site:nd.de`,
  ] : [
    `${topic.topic} site:theguardian.com OR site:nytimes.com OR site:npr.org`,
    `${topic.topic} site:washingtonpost.com OR site:lemonde.fr OR site:derstandard.at`,
  ];
...
ERLAUBTE QUELLEN (nur diese):
Deutsch: Spiegel, taz, Süddeutsche Zeitung, Zeit, Tagesspiegel, Frankfurter Rundschau, Stern
International: Guardian, New York Times, NPR, Washington Post, Le Monde

VERBOTENE QUELLEN:
Parteien (SPD, Grüne etc.), Blogs, World Socialist Web Site, unbekannte Seiten

Führe die Suche durch und extrahiere NUR Artikel von den erlaubten Medien.
Ignoriere alle anderen Treffer.

Gib NUR valides JSON zurück:
{"articles":[{"source":"Spiegel","title":"...","key_points":["Punkt 1","Punkt 2"]}],"search_successful":true}`,
    messages: [{
      role: "user",
  ...
  try {
    ...
    // Filtere unbekannte Quellen
    const allowedLeft = [
      // Deutsch links/liberal
      ...
      "Merkur", "Redaktionsnetzwerk", "T-Online", "Watson",
      // International links/liberal
    ...
    result.articles = (result.articles || []).filter(a =>
      allowedLeft.some(allowed => a.source?.toLowerCase().includes(allowed.toLowerCase()))
    );
...
async function agent3b_searchRight(topic, category) {
  ...
  // Präzise Suchanfragen mit bekannten Domains
  const searches = isGerman ? [
    `${topic.topic} site:faz.net OR site:welt.de OR site:nzz.ch OR site:focus.de`,
    `${topic.topic} site:cicero.de OR site:handelsblatt.com OR site:wiwo.de OR site:rp-online.de`,
  ] : [
    `${topic.topic} site:wsj.com OR site:telegraph.co.uk OR site:washingtonexaminer.com`,
    `${topic.topic} site:nzz.ch OR site:thetimes.co.uk OR site:spectator.co.uk`,
  ];
...
ERLAUBTE QUELLEN (nur diese):
Deutsch: FAZ, Welt, NZZ, Focus, Cicero, Handelsblatt
International: Wall Street Journal, Telegraph, Washington Examiner, The Times, The Spectator

VERBOTENE QUELLEN:
Breitbart, InfoWars, RT, Blogs, unbekannte Seiten, Parteiseiten

Führe die Suche durch und extrahiere NUR Artikel von den erlaubten Medien.
Ignoriere alle anderen Treffer.

Gib NUR valides JSON zurück:
{"articles":[{"source":"FAZ","title":"...","key_points":["Punkt 1","Punkt 2"]}],"search_successful":true}`,
    messages: [{
      role: "user",
  ...
  try {
    ...
    const allowedRight = [
      ...
      "Wirtschaftswoche", "WiWo", "Capital", "The Pioneer", "Tichys Einblick",
      "Junge Freiheit", "Rheinische Post", "RP Online",
      "Augsburger Allgemeine", "Münchner Merkur", "BILD", "Bild",
      ...
      "Neue Zürcher Zeitung", "NZZ International",
    ];
    result.articles = (result.articles || []).filter(a =>
      allowedRight.some(allowed => a.source?.toLowerCase().includes(allowed.toLowerCase()))
    );
...
async function agent4a_writeLeftPerspective(topic, facts, leftSearchResults) {
  ...
  const res = await withRetry(() => client.messages.create({
    ...
    max_tokens: 1000,
    system: `Du schreibst die links-liberale Perspektive für ein ausgewogenes Nachrichten-Briefing. (Datum: ${today})
...
async function agent4b_writeRightPerspective(topic, facts, rightSearchResults) {
  ...
  const res = await withRetry(() => client.messages.create({
    model: "claude-sonnet-4-20250514",
    max_tokens: 1000,
    system: `Du schreibst die konservative Perspektive für ein ausgewogenes Nachrichten-Briefing. (Datum: ${today})
...
FAZ, Welt, NZZ, WSJ, Telegraph sind konservative Quellen.
...
async function agent5_qualityCheck(item) {
  const res = await withRetry(() => client.messages.create({
    model: "claude-haiku-4-5-20251001",
    max_tokens: 300,
    system: `Du prüfst die Qualität eines Nachrichten-Briefings.
...
async function generateCategoryBriefing(categoryId) {
  ...
  const filename = `briefing-${categoryId}.json`;
