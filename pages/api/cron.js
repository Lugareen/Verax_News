...
async function agent3a_searchLeft(topic, category) {
  ...
  // Für Börsenthemen: einfachere Suche ohne Mediennamen
  const searches = isBoerse ? [
    `${topic.topic} aktuell 2026`,
    `${topic.topic} Analyse Kommentar`,
  ] : isGerman ? [
...
async function agent3b_searchRight(topic, category) {
  ...
  // Für Börsenthemen: einfachere Suche ohne Mediennamen
  const searches = isBoerse ? [
    `${topic.topic} Aktie Kurs 2026`,
    `${topic.topic} Analyse Prognose Kommentar`,
  ] : isGerman ? [
