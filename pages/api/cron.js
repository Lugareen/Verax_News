...
async function agent3a_searchLeft(topic, category) {
  ...
  // Allgemeine Suchanfragen — Medienfilterung erfolgt danach
  const searches = isBoerse ? [
    `${topic.topic} 2026`,
    `${topic.topic} Kommentar Einschätzung`,
  ] : isGerman ? [
    `${topic.topic} Deutschland 2026`,
    `${topic.topic} Kommentar Reaktion`,
  ] : [
    `${topic.topic} 2026`,
    `${topic.topic} analysis opinion`,
  ];
...
async function agent3b_searchRight(topic, category) {
  ...
  const searches = isBoerse ? [
    `${topic.topic} 2026`,
    `${topic.topic} Markt Prognose Analyse`,
  ] : isGerman ? [
    `${topic.topic} Deutschland 2026`,
    `${topic.topic} Kommentar Einschätzung`,
  ] : [
...
async function generateCategoryBriefing(categoryId) {
  ...
  for (const topic of topics) {
    ...
    if (!facts.headline || facts.headline.toLowerCase().includes("keine") || facts.headline.toLowerCase().includes("verfügbar")) {
      ...
      facts.summary = facts.summary || `Aktuelle Berichterstattung zu: ${topic.topic}`;
    }
    console.log(`  Agent 2: "${facts.headline}" (${facts.confidence})`);

    // Agents 3a + 3b PARALLEL — mit Pause davor und danach
    await pause(5000);
    ...
    // Agents 4a + 4b SEQUENTIELL mit Pause dazwischen
    await pause(10000);
