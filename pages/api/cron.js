...
async function agent1_selectTopics(primaryArticles, category) {
  // Max 40 Artikel statt 60 — weniger Tokens
  const list = primaryArticles.slice(0, 40).map((a, i) =>
    `[${i+1}] [${a.source}/${a.lean}] ${a.title}`
  ).join("\n");
...
❌ Internationale Themen die Deutschland nur nebenbei erwähnen
` : ""}
...
{"top_topics":[{"rank":1,"topic":"Max 8 Wörter","source_count":5,"sources":["BBC"],"representative_title":"Titel"}]}`,
...
Gib NUR valides JSON zurück — Format: {"headline":"...","summary":"...","confidence":"high/medium/low"}`,
...
{"left_perspective":{"paragraphs":["p1","p2"],"sources_used":["Spiegel"],"basis_tag":"article","key_argument":"1 Satz"},"right_perspective":{"paragraphs":["p1","p2"],"sources_used":["FAZ"],"basis_tag":"article","key_argument":"1 Satz"},"divergence_score":7,"divergence_note":"Kurz"}`,
  ...
  // Web Search Antwort verarbeiten
...
async function generateCategoryBriefing(category, primary, secondary, all) {
  ...
  const primaryArticles = primaryFiltered.length >= 3 ? primaryFiltered : primary.slice(0, 30);
  const allArticles = allFiltered.length >= 5 ? allFiltered : all.slice(0, 40);
