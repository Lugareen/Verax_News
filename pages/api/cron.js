...
async function agent3a_searchLeft(topic, category) {
  ...
  const searchQuery = isGerman
    ? `${topic.topic} Spiegel OR taz OR "Süddeutsche Zeitung" OR Zeit OR Tagesspiegel`
    : `${topic.topic} Guardian OR "New York Times" OR NPR OR "Washington Post" OR "Le Monde"`;

  const res = await withRetry(() => client.messages.create({
    model: "claude-haiku-4-5-20251001",
    max_tokens: 1500,
    tools: [{ type: "web_search_20250305", name: "web_search", max_uses: 3 }],
    system: `Du suchst nach Artikeln von links-liberalen Medien zu einem Nachrichtenthema.

PFLICHTSUCHE: "${searchQuery}"
Führe genau diese Suche durch.

Danach extrahiere aus den Ergebnissen:
- Welche Medien haben berichtet?
- Was sind die wichtigsten Aussagen und Zitate?
- Welche Aspekte werden betont?

Gib NUR valides JSON zurück:
{"articles":[{"source":"Spiegel","title":"...","key_points":["Punkt 1","Punkt 2"],"tone":"kritisch/besorgt/neutral"}],"search_successful":true}`,
    messages: [{ role: "user", content: `Thema: ${topic.topic}\nDatum: ${today}\n\nFühre die Pflichtsuche durch und extrahiere die wichtigsten Aussagen linker Medien.` }]
  }));
  ...
  try { return extractJSON(lastText); }
  catch { return { articles: [], search_successful: false }; }
}
...
async function agent3b_searchRight(topic, category) {
  ...
  const searchQuery = isGerman
    ? `${topic.topic} FAZ OR Welt OR NZZ OR Focus OR Cicero`
    : `${topic.topic} WSJ OR Telegraph OR "Washington Examiner" OR "National Review"`;

  const res = await withRetry(() => client.messages.create({
    model: "claude-haiku-4-5-20251001",
    max_tokens: 1500,
    tools: [{ type: "web_search_20250305", name: "web_search", max_uses: 3 }],
    system: `Du suchst nach Artikeln von konservativen Medien zu einem Nachrichtenthema.
...
Gib NUR valides JSON zurück:
{"articles":[{"source":"FAZ","title":"...","key_points":["Punkt 1","Punkt 2"],"tone":"kritisch/sachlich/neutral"}],"search_successful":true}`,
    messages: [{ role: "user", content: `Thema: ${topic.topic}\nDatum: ${today}\n\nFühre die Pflichtsuche durch und extrahiere die wichtigsten Aussagen konservativer Medien.` }]
  }));
