...
ANZAHL DER ABSÄTZE — WICHTIG:
Die Länge richtet sich nach dem verfügbaren Material:

• Volltext verfügbar (VOLLTEXT):     3 Absätze — viel Material, ausführliche Analyse
• Nur Teaser verfügbar (TEASER):     2 Absätze — begrenzt auf belegtes Material
• Keine Artikel dieser Seite:        1 Absatz  — nur redaktionelle Grundhaltung, klar als "knowledge" kennzeichnen

NIEMALS Absätze mit erfundenem Inhalt auffüllen um auf 3 zu kommen.
Qualität vor Länge — ein präziser Absatz ist besser als drei aufgeblähte.
...
STRUKTUR (je nach verfügbarem Material):
- Absatz 1: Wie bewertet diese Seite das Ereignis? Welchen Rahmen setzt sie?
- Absatz 2 (nur wenn genug Material): Welche Folgen und Aspekte betont sie?
- Absatz 3 (nur bei Volltext): Was fordert sie? Ihr Grundtenor.
...
LINKS/LIBERAL (${hasLeftArticles
  ? `${allArticles.filter(a=>a.lean==="left" && a.hasFullText).length} Volltexte, ${allArticles.filter(a=>a.lean==="left" && !a.hasFullText).length} Teaser → schreibe ${allArticles.filter(a=>a.lean==="left" && a.hasFullText).length > 0 ? "3" : allArticles.filter(a=>a.lean==="left").length > 0 ? "2" : "1"} Absatz/Absätze`
  : "KEINE Artikel → schreibe 1 Absatz, basis_tag: knowledge"}):
${leftArticles || "Keine Artikel verfügbar"}

KONSERVATIV (${hasRightArticles
  ? `${allArticles.filter(a=>a.lean==="right" && a.hasFullText).length} Volltexte, ${allArticles.filter(a=>a.lean==="right" && !a.hasFullText).length} Teaser → schreibe ${allArticles.filter(a=>a.lean==="right" && a.hasFullText).length > 0 ? "3" : allArticles.filter(a=>a.lean==="right").length > 0 ? "2" : "1"} Absatz/Absätze`
  : "KEINE Artikel → schreibe 1 Absatz, basis_tag: knowledge"}):
...
async function generateCategoryBriefing(category, primary, secondary, all) {
  ...
  const newsItems = (await Promise.all(
    topics.map(async (topic) => {
      ...
      // ── QUALITÄTS-GATE ───────────────────────────────────────────────────
      // Überspringe Thema wenn Agent 2 kein echtes Material gefunden hat
      const noMaterial = !facts.summary ||
        facts.summary.toLowerCase().includes("keine informationen") ||
        facts.summary.toLowerCase().includes("nicht verfügbar") ||
        facts.summary.toLowerCase().includes("keine artikel") ||
        facts.summary.toLowerCase().includes("no information") ||
        facts.confidence === "low";

      if (noMaterial) {
        console.log(`⚠️ Überspringe Thema "${topic.topic}" — zu wenig Material`);
        return null;
      }
      // ─────────────────────────────────────────────────────────────────────
