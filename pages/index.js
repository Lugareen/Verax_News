...
// Safe track wrapper — verhindert SSR-Fehler
const safeTrack = (event, props) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setData(json.data);
        setState("done");
        // Analytics: Kategorie wurde gelesen
        safeTrack("briefing_viewed", {
          category_id: cat.id,
    ...
    <div style={{ background:"#fff", border:"1px solid rgba(13,27,62,0.09)", marginBottom:2 }}>
      ...
      {state === "done" && data && (
        <div>
          ...
          {(data.items || data.news || []).map((item, i) => <NewsCard key={i} item={item} index={i} />)}
        </div>
      )}
    </div>
  );
...
function ResetPasswordModal({ onClose, onSuccess }) {
  ...
  const inputStyle = { width:"100%", padding:"12px 14px", border:"1.5px solid rgba(13,27,62,0.12)", borderRadius:8, fontSize:14, fontFamily:"'DM Sans', sans-serif", color:"#1A1A2A", background:"#FAFAF7", outline:"none", boxSizing:"border-box" };

  const handleReset = async () => {
    if (!password || !confirm) { setError("Bitte beide Felder ausfüllen."); return; }
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(13,27,62,0.65)", zIndex:300, display:"flex", alignItems:"center", justifyContent:"center", padding:24, backdropFilter:"blur(4px)" }}>
      <div style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:420, overflow:"hidden", boxShadow:"0 24px 64px rgba(13,27,62,0.2)", padding:"40px 36px" }}>
        {/* Header */}
        <div style={{ marginBottom:28 }}>
          <div style={{ fontFamily:"'Syne', sans-serif", fontSize:22, fontWeight:800, color:"#0D1B3E", marginBottom:8 }}>
            Neues Passwort setzen
          </div>
          <div style={{ fontSize:14, color:"#6B6B80", fontWeight:300 }}>
        ...
        {done ? (
          <div style={{ textAlign:"center", padding:"20px 0" }}>
            ...
            <div style={{ fontSize:15, color:"#0D1B3E", fontWeight:600 }}>Passwort erfolgreich geändert!</div>
            <div style={{ fontSize:13, color:"#6B6B80", marginTop:8 }}>Du wirst automatisch eingeloggt...</div>
          </div>
        ) : (
          <>
            <div style={{ marginBottom:16 }}>
              <label style={{ fontSize:12, fontWeight:600, color:"#6B6B80", letterSpacing:"0.05em", textTransform:"uppercase", display:"block", marginBottom:6 }}>Neues Passwort</label>
              <input
                type="password"
            ...
            <div style={{ marginBottom:24 }}>
              <label style={{ fontSize:12, fontWeight:600, color:"#6B6B80", letterSpacing:"0.05em", textTransform:"uppercase", display:"block", marginBottom:6 }}>Passwort bestätigen</label>
              <input
                type="password"
            ...
            <button
              onClick={handleReset}
              disabled={loading}
              style={{ width:"100%", padding:"14px", background:"#0D1B3E", color:"#fff", border:"none", borderRadius:10, fontSize:15, fontWeight:600, cursor: loading ? "not-allowed" : "pointer", fontFamily:"'DM Sans', sans-serif", opacity: loading ? 0.7 : 1 }}
            >
...
function AuthModal({ tab, onClose, onSwitch, onLogin }) {
  ...
  const handleForgotPassword = async () => {
    ...
    await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: "https://www.veraxnews.de",
    });
    setMessage("📧 Wir haben dir einen Reset-Link geschickt. Bitte prüfe deine E-Mails.");
  };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setUser(session?.user ?? null);
      // Erkennt wenn Nutzer über Reset-Link auf die Seite kommt
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", background:"#FAFAF7", fontFamily:"'DM Sans', sans-serif" }}>
        ...
        {modal === "reset-password" && (
          <ResetPasswordModal
            onClose={() => { setModal(null); setIsPasswordReset(false); }}
            onSuccess={() => { setModal(null); setIsPasswordReset(false); }}
          />
        )}
        {modal && modal !== "reset-password" && (
          <AuthModal tab={modal} onClose={() => setModal(null)} onSwitch={tab => setModal(tab)} onLogin={() => setModal(null)} />
        )}
        ...
        {isLoggedIn ? (
          ...
          <section className="vx-hero" style={{ minHeight:"100vh", display:"flex", flexDirection:"column", justifyContent:"center", position:"relative", overflow:"hidden", background:"linear-gradient(160deg, #FAFAF7 60%, #F2F0EB 100%)" }}>
            ...
            <div style={{ position:"relative", zIndex:1, maxWidth:800 }}>
              ...
              {/* Registrierungs-Anreiz */}
              <div style={{ display:"flex", alignItems:"flex-start", gap:12, marginTop:20, padding:"14px 18px", background:"rgba(201,147,58,0.08)", border:"1px solid rgba(201,147,58,0.25)", borderRadius:10, maxWidth:520 }}>
                <span style={{ fontSize:20, flexShrink:0 }}>🔓</span>
                <div>
                  <div style={{ fontSize:13, fontWeight:600, color:"#0D1B3E", marginBottom:2 }}>Nach der Registrierung sofort freigeschaltet:</div>
                  <div style={{ fontSize:13, color:"#6B6B80", fontWeight:300, lineHeight:1.6 }}>Alle 4 Kategorien · Politik & Wirtschaft International & Deutschland · Börsennachrichten International & Deutschland · kostenlos & ohne Kreditkarte</div>
                </div>
              </div>
          ...
          <div style={{ maxWidth:1100, margin:"0 auto" }}>
            ...
            {isLoggedIn ? (
              ...
              <div style={{ marginTop:32, display:"flex", flexDirection:"column", gap:12 }}>
                ...
                {/* Klarer Registrierungs-Anreiz */}
                <div style={{ background:"rgba(201,147,58,0.08)", border:"1px solid rgba(201,147,58,0.25)", borderRadius:10, padding:"16px 20px", display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:14, fontWeight:700, color:"#0D1B3E", marginBottom:4 }}>🔓 Alle 4 Kategorien kostenlos freischalten</div>
                    <div style={{ fontSize:13, color:"#6B6B80", fontWeight:300, lineHeight:1.6 }}>
                      Nach der kostenlosen Registrierung siehst du sofort: Politik Deutschland, Börsennachrichten International & Deutschland — ohne Kreditkarte.
                    </div>
                  </div>
                  <button onClick={() => setModal("register")} style={{ background:"#C9933A", color:"#fff", padding:"12px 24px", borderRadius:8, fontSize:14, fontWeight:700, border:"none", cursor:"pointer", fontFamily:"'DM Sans',sans-serif", whiteSpace:"nowrap" }}>
                    Jetzt registrieren →
                  </button>
                </div>
        ...
        <section id="preise" className="vx-section" style={{ background:"#F2F0EB" }}>
          <div style={{ maxWidth:1100, margin:"0 auto" }}>
            ...
            <h2 style={{ fontFamily:"'Syne', sans-serif", fontSize:"clamp(28px,4vw,44px)", fontWeight:800, color:"#0D1B3E", letterSpacing:"-0.02em", lineHeight:1.1, marginBottom:8 }}>Starte kostenlos.</h2>
            <p style={{ fontSize:17, color:"#6B6B80", fontWeight:300, marginBottom:48, maxWidth:520 }}>Registriere dich gratis und erhalte täglich das wichtigste internationale Briefing — ohne Kreditkarte.</p>
            <div className="vx-pricing-grid" style={{ marginTop:0 }}>
              {[
                {
                  name:"Free",
                  price:"€0",
                  period:"für immer kostenlos",
                  highlight: "Perfekt zum Einstieg",
                  features:[
                    "🌍 Politik & Wirtschaft International",
                    "🇩🇪 Politik & Wirtschaft Deutschland",
                    "📈 Börsennachrichten International",
                    "🏦 Börsennachrichten Deutschland",
                    "📊 Top 3 Nachrichten täglich um 06:00 Uhr",
                    "⚖️ Links/Rechts-Analyse je Nachricht",
                    "📰 Quellennachweis je Nachricht",
                    "📱 Mobile optimiert",
                  ],
                  upgrade: "Mit Premium: stündliche Updates, Podcast & Archiv",
                  featured:false,
                  ctaText: "Jetzt kostenlos registrieren →",
                  ctaActive: true,
                },
                {
                  name:"Premium",
                  ...
                  features:[
                    ...
                    "🎙️ Täglicher Podcast (5 Min.)",
                    "🗄️ Archiv (letzte 30 Tage)",
                ...
                },
                {
                  name:"Business",
                  price:"€149",
                  period:"pro Monat · bis 15 Nutzer",
                  highlight: "Für Teams",
                  features:[
                    "✅ Alle Premium-Features",
                    "👥 Bis 15 Nutzer inklusive",
                ...
                <div key={plan.name} style={{ background: plan.featured ? "#0D1B3E" : "#fff", padding:"44px 36px", display:"flex", flexDirection:"column", borderRadius: plan.featured ? 0 : 0 }}>
                  {/* Badge */}
                  <div style={{ marginBottom:16 }}>
                    <span style={{ background: plan.featured ? "#C9933A" : "rgba(13,27,62,0.06)", color: plan.featured ? "#fff" : "#6B6B80", fontSize:10, fontWeight:700, padding:"3px 10px", borderRadius:100, letterSpacing:"0.1em", textTransform:"uppercase" }}>
                      {plan.highlight}
                    </span>
                  </div>
                  {/* Name */}
                  ...
                  {/* Price */}
                  <div style={{ fontFamily:"'Syne', sans-serif", fontSize:44, fontWeight:800, color: plan.featured ? "#C9933A" : "#0D1B3E", letterSpacing:"-0.03em", lineHeight:1, margin:"12px 0 4px" }}>{plan.price}</div>
                  <div style={{ fontSize:13, color: plan.featured ? "rgba(255,255,255,0.4)" : "#B0B0C0", marginBottom:28 }}>{plan.period}</div>
                  <div style={{ height:1, background: plan.featured ? "rgba(255,255,255,0.1)" : "rgba(13,27,62,0.08)", marginBottom:24 }} />
                  {/* Features */}
                  <div style={{ display:"flex", flexDirection:"column", gap:10, flex:1 }}>
                    {plan.features.map(f => (
                      <div key={f} style={{ display:"flex", gap:8, fontSize:13, color: plan.featured ? "rgba(255,255,255,0.8)" : "#1A1A2A", fontWeight:300, lineHeight:1.5 }}>
                        {f}
                      </div>
                    ))}
                  </div>
                  {/* Upgrade Teaser */}
                  {plan.upgrade && (
                    <div style={{ marginTop:16, padding:"10px 14px", background:"rgba(201,147,58,0.08)", borderRadius:8, border:"1px solid rgba(201,147,58,0.2)", fontSize:12, color:"#C9933A", lineHeight:1.5 }}>
                      ⭐ {plan.upgrade}
                    </div>
                  )}
                  {/* CTA Button */}
                  <button
                    ...
                    onClick={() => plan.ctaActive ? setModal("register") : null}
                    style={{ marginTop:24, padding:"14px 24px", borderRadius:8, fontSize:14, fontWeight:600, cursor: plan.ctaActive ? "pointer" : "not-allowed", fontFamily:"inherit", border: plan.ctaActive ? "none" : "1.5px solid rgba(13,27,62,0.1)", background: plan.ctaActive ? "#0D1B3E" : "transparent", color: plan.ctaActive ? "#fff" : "#B0B0C0", opacity: plan.ctaActive ? 1 : 0.6, transition:"all .2s" }}>
                    {plan.ctaText}
                  </button>
                  {!plan.ctaActive && (
                    <div style={{ marginTop:8, textAlign:"center", fontSize:11, color: plan.featured ? "rgba(255,255,255,0.3)" : "#B0B0C0" }}>
