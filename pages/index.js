...
function PerspectivePanel({ data, side }) {
  ...
  return (
    <div style={{ borderRadius:10, padding:20, background:bg, border:`1px solid ${border}`, display:"flex", flexDirection:"column", gap:10, flex:1 }}>
      ...
      {/* Key argument als Einleitung — jetzt oben */}
      {data.key_argument && (
...
// ─── AUTH MODAL ───────────────────────────────────────────────────────────
function AuthModal({ tab, onClose, onSwitch }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(13,27,62,0.65)", zIndex:300, display:"flex", alignItems:"center", justifyContent:"center", padding:24, backdropFilter:"blur(4px)" }}>
      <div onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:420, overflow:"hidden", boxShadow:"0 24px 64px rgba(13,27,62,0.2)", position:"relative" }}>
        ...
        <div style={{ background:"#0D1B3E", padding:"28px 32px 24px", position:"relative" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:18 }}>
            ...
            <span style={{ fontFamily:"'Syne', sans-serif", fontSize:15, fontWeight:800, color:"#fff", letterSpacing:"-0.02em" }}>VERAX NEWS</span>
          </div>
          {/* Tab switcher */}
          <div style={{ display:"flex", gap:2, background:"rgba(255,255,255,0.08)", borderRadius:8, padding:3 }}>
            <button onClick={() => onSwitch("register")} style={{ flex:1, padding:"8px", borderRadius:6, border:"none", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", background:isRegister ? "#C9933A" : "transparent", color:isRegister ? "#fff" : "rgba(255,255,255,0.45)", transition:"all .2s" }}>Registrieren</button>
            <button onClick={() => onSwitch("login")} style={{ flex:1, padding:"8px", borderRadius:6, border:"none", fontSize:13, fontWeight:500, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", background:!isRegister ? "#C9933A" : "transparent", color:!isRegister ? "#fff" : "rgba(255,255,255,0.45)", transition:"all .2s" }}>Anmelden</button>
          </div>
            ...
            <>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                <div>
                  ...
                  <input type="email" placeholder="deine@email.de" style={inputStyle} />
                </div>
                <div>
                  ...
                  <input type="password" placeholder="Mindestens 8 Zeichen" style={inputStyle} />
                </div>
              </div>
              {/* Plan selection */}
              <div style={{ marginTop:18, display:"flex", flexDirection:"column", gap:8 }}>
                {[
                  ...
                  { id:"premium", label:"Premium", desc:"Stündlich aktualisiert · Täglicher Podcast", price:"€1,99/Mo", badge:"BELIEBT" },
                ].map(p => (
                  <div key={p.id} onClick={() => setPlan(p.id)} style={{ border:`2px solid ${plan===p.id ? (p.id==="premium" ? "#C9933A" : "#0D1B3E") : "rgba(13,27,62,0.12)"}`, borderRadius:10, padding:"11px 14px", cursor:"pointer", display:"flex", alignItems:"center", gap:12, background:plan===p.id ? (p.id==="premium" ? "rgba(201,147,58,0.04)" : "rgba(13,27,62,0.03)") : "transparent", transition:"all .2s" }}>
                    <div style={{ width:17, height:17, borderRadius:"50%", border:`2px solid ${plan===p.id ? (p.id==="premium" ? "#C9933A" : "#0D1B3E") : "rgba(13,27,62,0.2)"}`, background:plan===p.id ? (p.id==="premium" ? "#C9933A" : "#0D1B3E") : "transparent", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                      {plan===p.id && <div style={{ width:6, height:6, borderRadius:"50%", background:"#fff" }} />}
                    </div>
                    <div>
                      <div style={{ fontSize:13, fontWeight:600, color:"#0D1B3E" }}>{p.label} {p.badge && <span style={{ background:"rgba(201,147,58,0.15)", color:"#C9933A", fontSize:9, fontWeight:700, padding:"1px 6px", borderRadius:100, letterSpacing:"0.1em", verticalAlign:"middle" }}>{p.badge}</span>}</div>
                      <div style={{ fontSize:11, color:"#6B6B80", fontWeight:300 }}>{p.desc}</div>
                    </div>
                    <div style={{ marginLeft:"auto", fontSize:13, fontWeight:700, color:p.id==="premium" ? "#C9933A" : "#0D1B3E" }}>{p.price}</div>
                  </div>
                ))}
              </div>
              <button style={{ width:"100%", marginTop:18, padding:13, background:"#0D1B3E", color:"#fff", border:"none", borderRadius:10, fontSize:14, fontWeight:500, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
                Jetzt registrieren →
              </button>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                <div>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:5 }}>
                    ...
                    <a href="#" style={{ fontSize:11, color:"#C9933A", textDecoration:"none" }}>Vergessen?</a>
                  </div>
                  <input type="password" placeholder="••••••••" style={inputStyle} />
                </div>
              </div>
              <button style={{ width:"100%", marginTop:20, padding:13, background:"#0D1B3E", color:"#fff", border:"none", borderRadius:10, fontSize:14, fontWeight:500, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
                Anmelden →
              </button>
              <div style={{ textAlign:"center", marginTop:18, fontSize:13, color:"#6B6B80" }}>
                Noch kein Konto?{" "}
                <button onClick={() => onSwitch("register")} style={{ background:"none", border:"none", color:"#0D1B3E", fontWeight:500, cursor:"pointer", fontSize:13, fontFamily:"'DM Sans',sans-serif" }}>Jetzt registrieren</button>
              </div>
            </>
          )}
        </div>
  ...
  return (
    <>
      ...
      <div style={{ minHeight:"100vh", background:"#FAFAF7", fontFamily:"'DM Sans', sans-serif" }}>
        ...
        {/* ── AUTH MODAL ── */}
        {modal && <AuthModal tab={modal} onClose={() => setModal(null)} onSwitch={tab => setModal(tab)} />}
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:2, marginTop:48 }}>
              {[
                ...
                <div key={plan.name} style={{ background: plan.featured ? "#0D1B3E" : "#fff", padding:"44px 36px", display:"flex", flexDirection:"column" }}>
                  ...
                  <button onClick={() => setModal(plan.name === "Business" ? null : "register")} style={{ marginTop:28, padding:"14px 24px", borderRadius:8, fontSize:14, fontWeight:500, cursor:"pointer", fontFamily:"inherit", border:"none", background: plan.featured ? "#C9933A" : "transparent", color: plan.featured ? "#fff" : "#0D1B3E", borderWidth: plan.featured ? 0 : 1.5, borderStyle:"solid", borderColor:"rgba(13,27,62,0.2)" }}>
                    {plan.name === "Free" ? "Kostenlos registrieren" : plan.name === "Standard" ? "Jetzt abonnieren" : "Demo anfragen"}
                  </button>
                </div>
              ))}
            </div>
