  ...
  return (
    <div style={{ background:"#fff", padding:"32px 40px", borderBottom:"1px solid rgba(13,27,62,0.06)", display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={{ paddingLeft:44 }}>
        <div className="perspectives" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
  ...
  return (
    <>
      ...
      <div style={{ minHeight:"100vh", background:"#FAFAF7", fontFamily:"'DM Sans', sans-serif" }}>
        <style>{`*{box-sizing:border-box;margin:0;padding:0} @keyframes spin{to{transform:rotate(360deg)}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}} @media(max-width:700px){.perspectives{grid-template-columns:1fr !important}}`}</style>
        ...
        <section style={{ minHeight:"100vh", padding:"140px 64px 80px", display:"flex", flexDirection:"column", justifyContent:"center", position:"relative", overflow:"hidden", background:"linear-gradient(160deg, #FAFAF7 60%, #F2F0EB 100%)" }}>
          ...
          <div style={{ position:"relative", zIndex:1, maxWidth:800 }}>
            ...
            <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}>
              {isLoggedIn ? (
                <a href="#briefing" style={{ background:"#0D1B3E", color:"#fff", padding:"16px 36px", borderRadius:10, fontSize:15, fontWeight:500, textDecoration:"none", display:"inline-block" }}>Heutiges Briefing lesen →</a>
              ) : (
                <>
        ...
        <section id="briefing" style={{ background:"#F2F0EB", padding:"100px 64px" }}>
          <div style={{ maxWidth:1100, margin:"0 auto" }}>
            ...
            {/* Politik & Wirtschaft International — immer sichtbar */}
              ...
              {[
                ...
                <div key={plan.name} style={{ background: plan.featured ? "#0D1B3E" : "#fff", padding:"44px 36px", display:"flex", flexDirection:"column" }}>
                  ...
                  <button onClick={() => setModal(plan.name === "Business" ? null : "register")} style={{ marginTop:28, padding:"14px 24px", borderRadius:8, fontSize:14, fontWeight:500, cursor:"pointer", fontFamily:"inherit", border:"none", background: plan.featured ? "#C9933A" : "transparent", color: plan.featured ? "#fff" : "#0D1B3E", borderWidth: plan.featured ? 0 : 1.5, borderStyle:"solid", borderColor:"rgba(13,27,62,0.2)" }}>
                    {plan.name === "Free" ? "Kostenlos registrieren" : plan.name === "Premium" ? "Jetzt abonnieren" : "Demo anfragen"}
                  </button>
                </div>
              ))}
            </div>
