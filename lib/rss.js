...
async function fetchFeed(source) {
  try {
    ...
    // Volltext parallel für bis zu 3 Artikel pro Quelle abrufen
    const articles = await Promise.all(items.map(async (item, idx) => {
      // Volltext nur für erste 3 Artikel pro Quelle (Geschwindigkeit)
      const fullText = idx < 3
